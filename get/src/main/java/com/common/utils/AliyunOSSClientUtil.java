...
public class AliyunOSSClientUtil {
    ...
    public static String uploadOSS(MultipartFile uploadFile, String filename) throws Exception {
        // 云账号AccessKey有所有API访问权限，建议遵循阿里云安全最佳实践，创建并使用RAM子账号进行API访问或日常运维，请登录https://ram.console.aliyun.com 创建
        logger.info("阿里云OSS上传............"+uploadFile.getOriginalFilename());
        // 创建OSSClient实例
        OSSClient ossClient = new OSSClient(ENDPOINT, ACCESS_KEY_ID, ACCESS_KEY_SECRET);
        // 上传
        ossClient.putObject(BACKET_NAME, FOLDER+filename, new ByteArrayInputStream(uploadFile.getBytes()));
        // 关闭client
        ossClient.shutdown();
        // 设置一百天有效
        Date expiration = new Date(System.currentTimeMillis() + 3600L * 1000 * 24 * 100);
        String url = ossClient.generatePresignedUrl(BACKET_NAME, FOLDER+filename, expiration).toString();
...
//        }
        // 创建bucketName
//        String settlement =
//                createBucketName(ossClient, "settlement");
...
//        System.out.println(folder);
        ...
        try {
            ...
            images = uploadObject2OSS(ossClient,new File("D:\\image\\001.jpg"),BACKET_NAME,FOLDER);
        } catch (Exception e) {
            e.printStackTrace();
        }
