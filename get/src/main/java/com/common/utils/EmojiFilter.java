...
public class EmojiFilter {
    ...
    public static Boolean filterEntityEmoji(Object entity) {
        Field[] fields = entity.getClass().getDeclaredFields();
        Boolean contailsIllegalChar = false;
        for(Field e:fields){
            Class<?> type = e.getType();
            if(type.getTypeName().equals("java.lang.String")){
                e.setAccessible(true);
                try {
                    Object o = e.get(entity);
                    if(o == null){
                        continue;
                    }
                    ...
                    for (int i = 0; i < str.length(); i++) {
                        ...
                        if(EmojiFilter.isEmojiCharacter(c) || String.valueOf(c).contains("[^\\u0000-\\uFFFF]")){
                            contailsIllegalChar = true;
                        }
                        /*Matcher matcher = CHECK_EMOJI.matcher(temp);
                        if (matcher.find()) {
                            contailsIllegalChar = true;
                        }*/
                    }
